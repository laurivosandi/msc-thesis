...
\begin{tikzpicture}
    \begin{axis}[xbar,width=12cm, height=8cm,
...
mounted as \texttt{tmpfs} to exclude the
HDD/SDD effects on the test.
...
\begin{tikzpicture}
    \begin{axis}[xbar,width=12cm, height=8cm,
        ...
        symbolic y coords={xz, raw over 100MBps, bzip2, pxz, gzip, raw over 300MBps, raw over 1GBps, pigz},
        nodes near coords, nodes near coords align={horizontal},
        ytick=data]
        \addplot coordinates {
            (46,pigz)
            (57.5,raw over 1GBps)
            (192,raw over 300MBps)
            (206,gzip)
            (494,pxz)
            (555,bzip2)
            (575,raw over 100MBps)
            (2150,xz)
        };
    \end{axis}
\end{tikzpicture}
...
Estonian Educational Network has planned infrastructure
